...
def plot_polygon(coords, ax, blname, yn_value, face_color: str = None):
    ...
    ax.text(centroid[0], centroid[1], blname, ha='center', va='center', fontsize=7, color='black')
...
def full(ax, block_items: dict, route_plan: list):
...
def draw(block_items: dict, route_plan: list):
    ...
    while True:
        match input("n: 다음, p: 이전, 타임라인 이동: t, 할당셀 그리기: a, 경로 전체 보기: 'f', 블럭 좌표 확인: b, 종료: '0': "):
            case 'p':
                print("이전")
